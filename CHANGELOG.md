...
- **Map view: selected-image corner highlight now uses the theme accent color**. When entering edit mode on a custom-map image, clicking the image to select it draws a halo (CSS box-shadow) around the image marking it as the active object. That halo was rendering in the leaflet library's hardcoded yellow (`#ffea00`) instead of `var(--interactive-accent)`, despite a theme-aware re-implementation in `map-view.css` trying to override it. Root cause was a cascade-order conflict — the theme-aware rule was silently overridden by the bundled library CSS due to component concatenation order. The fix consolidates the relevant rules into a single theme-aware block in `leaflet-distortable.css`, scoped under `.cr-map-view`. Most of the other rules in the consolidation (rotate / scale / distort / lock toolbar icons, keymapper panel, toggle / cancel buttons) cover leaflet-distortable's default popup toolbar, which is intentionally suppressed in Charted Roots' map view via `suppressToolbar: true`; theme alignment there is defensive in case the suppression is ever flipped off.

### Changed

- **Internal: leaflet-distortable CSS dedupe**: 25 duplicate-selector cluster between `styles/map-view.css` and `styles/leaflet-distortable.css` consolidated to a single theme-aware block in `leaflet-distortable.css` with `.cr-map-view` scoping. Closes the largest category in the Community automated review's v0.22.38 CSS lint surface (the `.ldi-*`, `.leaflet-toolbar-icon.*`, `#toggle-keymapper`, etc. selectors). styles.css drops by 274 lines (41,244 → 40,970). Plan documented at [`docs/planning/leaflet-distortable-dedupe-plan.md`](docs/planning/leaflet-distortable-dedupe-plan.md).
