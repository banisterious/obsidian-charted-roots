...
- **A custom calendar now wins over a built-in that shares its era abbreviations** ([#650](https://github.com/banisterious/obsidian-charted-roots/issues/650)): The previous fix routed "Systems in use" through the shared parser with the note's universe, but a built-in date system carries its own universe and is checked first, so a custom calendar reusing the built-in's era abbreviations (e.g. a Star Wars calendar reusing `BBY` / `ABY`) was still shadowed — the breakdown credited every date to the built-in "Galactic Standard Calendar". The parser now prefers a user-defined system over a built-in that shares the same era abbreviation (and matches the universe regardless of casing or wikilink wrapping), so dates are attributed to the custom calendar. Reported by [@DigitalDreamn](https://github.com/DigitalDreamn).
