...
- **Custom relationship overlay lines now arc under the tree instead of drawing straight across cards** ([#404](https://github.com/banisterious/obsidian-charted-roots/issues/404)): On larger trees a straight overlay line between two card centers would cut diagonally through intervening cards, and for pairs of distant relatives it could trace along existing parent-child paths and imply the overlay relationship flowed through a shared ancestor. Overlays now draw as quadratic bezier curves that arc below the chord, with sag scaled to chord length. Curvature reads as "not a family line" regardless of where the endpoints sit, color and dash pattern still carry the relationship type, and the overlay still paints beneath the family-link layer.
- **Adopted / step / foster overlays now restyle the existing parent-child link instead of drawing a parallel arc** ([#404](https://github.com/banisterious/obsidian-charted-roots/issues/404)): When one of these built-in overlay types maps onto a pair that also has a structural parent-child link in the visible tree, the structural link itself is now colored with the overlay type's stroke and dash pattern, and carries the overlay's tooltip — rather than drawing a separate arc alongside it. The viewer sees one expressive line per relationship instead of two coincident ones. Pairs without a visible structural link (e.g., the adoptive parent isn't currently in the chart due to depth or root settings) fall back to the arc rendering. Purely custom overlays (sire / childer, mentor, ally, rival, etc.) still arc since they don't have a structural counterpart.
