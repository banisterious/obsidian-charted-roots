...
- **Map view: custom-image edit-mode toolbar and keymapper now render with theme-aware colors** (latent visual bug, no prior reporter). The distortable-image controls — rotate / scale / drag / distort / lock toolbar icons, the keymapper panel, the toggle and cancel buttons — were rendering with the leaflet library's hardcoded colors (blue `#0078a8` icons, pure-white keymapper background even in dark mode, `21px` border-radius vs Obsidian's `var(--radius-l)`) instead of the theme-aware values that `map-view.css` had been trying to apply. Root cause was a cascade-order conflict: the theme-aware re-implementation in `map-view.css` was being overridden by the bundled library CSS due to component concatenation order. Resolution consolidates the rules into a single theme-aware block in `leaflet-distortable.css`, scoped under `.cr-map-view` for containment.
