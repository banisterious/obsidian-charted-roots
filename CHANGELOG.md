...
- **"Repair misaligned children" now catches a co-parent the first pass could miss** ([#666](https://github.com/banisterious/obsidian-charted-roots/issues/666)): When two parents shared the same children, the repair tool could flag one parent yet report the other as clean if that parent's children list happened to stay the same length as its companion ID list. In that state a broken child link can quietly mask a real child — the link points nowhere while its paired ID belongs to a different person — so the note looked aligned by count but wasn't. The detector now treats a broken child link as a repair trigger in its own right, so both parents are flagged and the masked children are recovered. Reported by [@DigitalDreamn](https://github.com/DigitalDreamn).
