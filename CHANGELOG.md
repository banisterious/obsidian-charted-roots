...
- **Map event pop-ups show the event type's display name and icon** ([#774](https://github.com/banisterious/obsidian-charted-roots/issues/774)): in the Map view, an event whose type had a two-word internal id (e.g. `plot_point`) showed the raw slug ("Plot_point") with no icon in the marker pop-up — single-word types happened to look right, which hid it. The journey-mode pop-ups had the same gap. The pop-ups now resolve the event type's display name and icon from the real type (falling back to a readable form of the id for an unregistered type), so a custom or two-word event type shows its proper name and icon. Reported by [@DigitalDreamn](https://github.com/DigitalDreamn).
- **Newly created places appear in the Places tab list right away** ([#772](https://github.com/banisterious/obsidian-charted-roots/issues/772) follow-up): the #772 fix made the Control Center's "Place notes" list wait for the metadata cache when a place was *edited*, but creating a place — from the "Create place note" button, or the quick-create from an unlinked place name — still rebuilt the list before the new note was indexed, so it was missing from the list and its search until a manual refresh. Both create paths now wait for the new note to be indexed before refreshing, matching the edit paths. Reported by [@DigitalDreamn](https://github.com/DigitalDreamn).
- **Place hierarchy type legend lists every type on the chart, and the depth gradient shows its mid-tones** ([#767](https://github.com/banisterious/obsidian-charted-roots/issues/767) follow-up): in the Place hierarchy visualization, the "Color by: Place type" legend showed a fixed five-item list (country, state, city, town, village) while the chart colored every built-in type, so types like continent, region, estate, and church appeared on the chart but not in the legend. The legend now lists the place types actually present, in hierarchy order, with custom or untyped places grouped under a single "Other" entry. Two built-in types that had no color (planet, township) now get their own. The Depth legend's gradient also runs through its cyan/green/yellow mid-tones instead of only blue and red, matching the chart. Reported by [@DigitalDreamn](https://github.com/DigitalDreamn).
...
- **Hand-grouped families now merge in the Collections tab** ([#761](https://github.com/banisterious/obsidian-charted-roots/issues/761)): people grouped by a manually-set group name (rather than biological links) appeared as separate one-person families in the Control Center's Collections tab, even though the Person Picker merged them correctly. The Collections tab (and its analytics, like the "smallest" collection) now merge components that share a group name, matching the Person Picker. Reported by [@DigitalDreamn](https://github.com/DigitalDreamn).
- **Default universe now applies to people created outside the Create Person modal** ([#751](https://github.com/banisterious/obsidian-charted-roots/issues/751) follow-up): the default universe setting was only applied by the four "Create X" modals, so people created through the Create Family wizard — or added inline from the Edit Person modal — were left without it. Both paths now apply the default universe to brand-new people, matching the modals. Reported by [@DigitalDreamn](https://github.com/DigitalDreamn).
