...
- **Internal: Phase 4b mobile-migration first batch (`map-view.css`)**: First per-component migration consuming the v0.22.49 `MobileClassManager` infrastructure. The Map View's narrow-viewport toolbar rules (originally [#528](https://github.com/banisterious/obsidian-charted-roots/issues/528)) and the Control Center Maps tab's `world-map-preview` thumbnail rules now reach via dual paths: the original `@media (max-width: …)` blocks stay in place for narrow desktop / tablet leaves, and parallel `.cr-phone` blocks (using the global class applied by `MobileClassManager`) handle phones, where Obsidian's media queries don't fire reliably. The view-local `cr-map-view-phone` class consolidates into the global `.cr-phone`. User-visible side effect: phones whose viewport happens to be wider than 600px (some devices in landscape) now also get the compact `world-map-preview` layout that was previously gated on viewport width alone.
