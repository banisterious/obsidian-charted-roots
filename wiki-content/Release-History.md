...
- [v0.22.x](#v022x)
  - [v0.22.76 Round-Up: Map Timeline and Year-Range Filter Fixes, Journey-Map Place Hierarchy, and Place Display Names](#v02276-round-up-map-timeline-and-year-range-filter-fixes-journey-map-place-hierarchy-and-place-display-names-v02276)
  ...
  - [v0.22.70 Round-Up: Calendarium Integration Fix and Settings Relocation, Place-Linking by Note Type, and Era-Aware Statistics](#v02270-round-up-calendarium-integration-fix-and-settings-relocation-place-linking-by-note-type-and-era-aware-statistics-v02270)
  - [v0.22.69 Round-Up: GEDCOM Date-Format Import, an Inspectable Sort-Order Result, and Era-Aware Date Precision](#v02269-round-up-gedcom-date-format-import-an-inspectable-sort-order-result-and-era-aware-date-precision-v02269)
  ...
  - [v0.22.62 Round-Up: Stats Issues Card, Split Wizard Layout, Circle Ring, and the 1.13.0 Settings API](#v02262-round-up-stats-issues-card-split-wizard-layout-circle-ring-and-the-1130-settings-api-v02262)
  - [v0.22.61 Round-Up: Event-Driven Migration Analysis, Historical Place Names, and Family Chart Pop-Out Fixes](#v02261-round-up-event-driven-migration-analysis-historical-place-names-and-family-chart-pop-out-fixes-v02261)
  ...
  - [v0.22.56 Round-Up: Fictional-Date Event Count and jspdf Bundle Patch](#v02256-round-up-fictional-date-event-count-and-jspdf-bundle-patch-v02256)
  - [v0.22.55 Round-Up: Dependency Security Bump (jspdf and dompurify)](#v02255-round-up-dependency-security-bump-jspdf-and-dompurify-v02255)
  - [v0.22.54 Round-Up: Statistics and Relationships Polish, Timeline Era-Aware Sort, and Settings Reorg](#v02254-round-up-statistics-and-relationships-polish-timeline-era-aware-sort-and-settings-reorg-v02254)
  - [v0.22.53 Round-Up: Family Chart Overlay Self-Heal, Adoption Icon Continuity, and First External Contribution](#v02253-round-up-family-chart-overlay-self-heal-adoption-icon-continuity-and-first-external-contribution-v02253)
...
### v0.22.76 Round-Up: Map Timeline and Year-Range Filter Fixes, Journey-Map Place Hierarchy, and Place Display Names (v0.22.76)

A reporter-driven bug-fix release focused on the Maps and Places workflows. The map's timeline slider and year-range filter now work correctly on fictional universes, journey events map to their specific place rather than the whole country, and the Place hierarchy visualization's legend tracks the selected color mode. Place types and categories show their display names instead of internal ids across the place pickers and tools, the Places tab list refreshes promptly after edits, and a cluster of Control Center layout papercuts are cleared. Ten issues, nine reported by [@DigitalDreamn](https://github.com/DigitalDreamn) and the journey-map hierarchy fix by [@tenephor](https://github.com/tenephor).

**Fix: Collection highlights now merge hand-grouped families and skip lone notes** ([#761](https://github.com/banisterious/obsidian-charted-roots/issues/761) follow-up): The Control Center's "Collection highlights" (largest / smallest collection) and family count still treated people hand-grouped under a shared group name as separate one-person families, so a group could appear as the "smallest collection" with 1 person even though the Detected families table correctly merged it into one. The highlights and family count now apply the same group-name merge as the table. The largest/smallest highlights also no longer surface a single unconnected note (a one-person "Unnamed Family"), so they report the smallest and largest collections that actually group more than one person. This completes the #761 merge that the original ship missed on the analytics surface. Reported by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Fix: Collections pane now fills the available space** ([#762](https://github.com/banisterious/obsidian-charted-roots/issues/762)): The dockable Collections view's family list was capped at a fixed height — it reused the Control Center modal's height-limited list — so it stayed small in a wide tab or a tall sidebar instead of expanding to fill the pane like the other dockable views. The list now grows to fill the available area. Reported by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Fix: Journey map plots events at the right place, not the whole country** ([#763](https://github.com/banisterious/obsidian-charted-roots/issues/763)): On a person's journey map, events that named a hierarchical location (e.g. "Lebanon, Kansas, United States") were mapped to the top of the hierarchy — the country — instead of the specific city, while birth and death mapped correctly. The place lookup now resolves a hierarchical name to its most specific known place, so those events land on their actual city. Reported by [@tenephor](https://github.com/tenephor).

**Fix: Map timeline slider stays hidden until you open it** ([#764](https://github.com/banisterious/obsidian-charted-roots/issues/764)): The map's timeline slider is meant to appear only after pressing the Timeline button, but a styling conflict left it permanently visible — showing stale real-world 1800-2000 bounds with no era labels, ignoring the Timeline button's hide, and not refreshing its range when the Collection filter changed. The slider is now hidden by default and opens with the correct era-aware date range for the active map (and re-syncs as filters change). Reported by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Fix: Map year-range filter works on fictional maps** ([#765](https://github.com/banisterious/obsidian-charted-roots/issues/765)): The Map view's "From year" / "To year" filters were number-only, so a fictional-era value like "10 ABY" or "896 BBY" couldn't be entered — and a bare number was compared against the signed canonical years fictional events resolve to, which filtered every event off a fictional map. The fields now accept era dates and resolve them the same way the events do (a plain number still works for real-world maps), so year-range filtering behaves the same on fictional and real-world maps. Reported by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Fix: Place timeline margin and calendar-filter names** ([#766](https://github.com/banisterious/obsidian-charted-roots/issues/766)): The Place timeline card on the Control Center's Maps tab had its left/right padding zeroed, so the place picker and event rows rode the left edge instead of lining up with the surrounding cards — the horizontal margin is restored to match. The calendar filter dropdown (on the place and person timelines) also showed each calendar's internal id (e.g. `star_wars_out_of_universe_calendar`); it now shows the calendar's display name, falling back to a readable form of the id for any unnamed calendar. Reported by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Fix: Place hierarchy legend updates with the "Color by" mode** ([#767](https://github.com/banisterious/obsidian-charted-roots/issues/767)): In the Place hierarchy visualization (Maps tab → View hierarchy), switching "Color by" between Category, Place type, and Depth recolored the chart but left the legend showing the Category colors. The legend now rebuilds to match the selected mode. The control row (Layout / Color by / Show migrations) is also vertically centered now, so the Layout control no longer sits higher than the others. Reported by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Fix: Extract events modal no longer overlaps the toggle and remove button** ([#768](https://github.com/banisterious/obsidian-charted-roots/issues/768)): In the "Extract events from source" modal, each event's "Include this event" toggle sat directly underneath the remove (X) button in the top-right corner. The header now reserves space for the X, so the toggle and button no longer overlap. Reported by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Fix: Place type and category names shown instead of slug ids in the place picker and related modals** ([#770](https://github.com/banisterious/obsidian-charted-roots/issues/770)): The location tags in the "Select place" picker showed the internal ids (e.g. `astro_sector`, `fictional`) instead of the display names. They now resolve to the proper place-type name and category label. The same place-type display-name fix was applied to the other places that showed the raw id — the enrich place hierarchy preview, the cleanup wizard's hierarchy list, and the merge duplicate places modal. Reported by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Fix: Places tab list refreshes correctly after editing a place** ([#772](https://github.com/banisterious/obsidian-charted-roots/issues/772)): Editing a place from the Control Center's Places tab — for example setting or changing its place type — could make the note vanish from the "Place notes" list (and from its search) until the Control Center was reopened. The list rebuilt before Obsidian had finished re-indexing the saved note, so it read stale frontmatter. It now waits for the edit to be indexed before refreshing. Reported by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Stability-window impact:** v0.22.76's changes are all bug fixes (Maps, Places, and Collections papercuts) with no breaking changes, all within the v0.22.59 stability window, so the window holds at its v0.22.59 anchor (2026-06-02).
...
A worldbuilding-and-maps release. Spouse relationships can now record the type of union, place-type hierarchies are easier to grow upward, and a cluster of map and Entity Profile fixes makes mixed real-world/fictional vaults behave the way you'd expect. The bundled dompurify is also refreshed to clear a fresh security advisory.

**Added: Marriage type field** ([#628](https://github.com/banisterious/obsidian-charted-roots/issues/628)): Spouse relationships can now record the type of union — Common-law marriage, Cohabitation, Domestic (civil) partnership, Putative marriage, Concubinage, and the like — alongside the existing marriage date, location, and status. The Person modal offers these as quick-pick presets plus a "Custom..." option for any free-text value, and the type is mirrored to both partners. When set, it appears on Dynamic Timeline marriage rows — for example "Marriage to Jane Doe (Common-law marriage)" — controlled by a new "Show marriage type" setting (on by default). Requested by [@Vericia](https://github.com/Vericia).

**Added: Entity Profile memberships are sorted by start date** ([#743](https://github.com/banisterious/obsidian-charted-roots/issues/743)): A person's memberships were shown in insert order — and an edited membership jumped to the bottom — leaving date ranges scattered. They now order by earliest start year, resolved era-aware so fictional BBY/ABY dates sort by true chronology, with undated memberships last. On a tied start year an ended membership sorts above an ongoing ("Current") one, then ties break alphabetically by organization. Raised by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Changed: Editing a person now ensures `cr_type: person`** ([#744](https://github.com/banisterious/obsidian-charted-roots/issues/744)): Saving in the Edit Person modal now fills in a missing `cr_type: person` automatically. A note that gained a `cr_id` but never ran "Add essential person properties" could be edited indefinitely without ever being tagged as a person, which left type detection guessing. An existing `cr_type` is never overwritten. Requested by [@doctorwodka](https://github.com/doctorwodka).

**Changed: Place type hierarchy now inserts above an occupied level** ([#734](https://github.com/banisterious/obsidian-charted-roots/issues/734) follow-up): Creating or re-levelling a place type onto a level another type already holds now offers a choice — keep them tied, or insert above and push the lower types down by one. The previously fiddly "add something above the current top" case (e.g. a galaxy above a region) becomes a single action instead of renumbering the whole category by hand, while intentional ties and gaps are left intact. The per-row Hide/Show and Reset controls are now compact icons (eye and revert arrow) with tooltips, matching the customize/edit icons. Follow-up from testing by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Fix: Fictional events no longer appear on the real-world map at 0,0** ([#747](https://github.com/banisterious/obsidian-charted-roots/issues/747)): In a mixed vault, events tied to fictional places (which use pixel coordinates on a custom map) leaked onto the real-world map and clustered at latitude/longitude 0,0 off the coast of Africa, because their lat/long defaulted to zero. Markers, place markers, the heat map, and migration paths now render only the locations that belong to the active map's coordinate system, so real-world and fictional maps each show only their own places. Reported by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Fix: Entity Profile shows a place's category label, not its raw id** ([#745](https://github.com/banisterious/obsidian-charted-roots/issues/745)): The Category in a place's profile heading displayed the internal lowercase id (e.g. `historical`) instead of its display label ("Historical") — the label only appeared once you clicked into the field to edit it. The heading now shows the proper label, while editing still round-trips the underlying id. Reported by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Fix: Map data failures now show the actual error** ([#746](https://github.com/banisterious/obsidian-charted-roots/issues/746)): When the map failed to refresh, it showed only a generic "Failed to load map data" and logged the error as an empty object, making platform-specific failures impossible to diagnose. The specific error message now appears in both the notice and the log. Reported by [@tenephor](https://github.com/tenephor).

**Security: Bundled dompurify updated to 3.4.11** (transitive via jspdf) to resolve [GHSA-cmwh-pvxp-8882](https://github.com/advisories/GHSA-cmwh-pvxp-8882) (moderate; affects dompurify `<= 3.4.10`), a newly-disclosed follow-on to the hook-pollution issue addressed by the 3.4.10 bump in 0.22.72.

**Stability-window impact:** v0.22.73's changes are additive items (the marriage type field and date-sorted memberships) plus bug fixes and a dependency security bump (the real-world/fictional map separation, the place-category label, the surfaced map error, the `cr_type` safeguard, and the place-hierarchy insert-above refinement), with no breaking changes, all within the v0.22.59 stability window, so the window holds at its v0.22.59 anchor (2026-06-02).

### v0.22.72 Round-Up: Two 0.22.71 Regressions, a Timeline Crash, and Place-Type Reorder Refinements (v0.22.72)

A reporter-driven patch fixing two 0.22.71 regressions and a crash, plus place type refinements and a dependency security update. Organization member lists once again include person notes identified only by `cr_id`, the timeline no longer crashes on a bare-number date, place type reordering preserves intentional ties and gaps, and the parent-place dropdown shows display names. **1533 tests passing across 134 suites**.

**Fix: Organization members that are person notes without an explicit `cr_type` are listed again** ([#742](https://github.com/banisterious/obsidian-charted-roots/issues/742)): A regression in 0.22.71 (from the #738 fix) made the org member scan require an explicit `cr_type: person` / `type: person`, so person notes relying on the long-supported "`cr_id` and no explicit type" shape were silently dropped from their organizations' member lists — often leaving only one member visible. The scan now uses the shared person detection, which honours that legacy shape while keeping `cr_type` authoritative (so a foreign `type: character` is still ignored, preserving the #738 fix). Reported by [@doctorwodka](https://github.com/doctorwodka).

**Fix: Timeline no longer errors on a bare-number date** ([#741](https://github.com/banisterious/obsidian-charted-roots/issues/741)): A person note that combined an inline `events:` array with a date written as a bare number (e.g. `born: 1850`, which YAML parses as a number rather than text) crashed the whole timeline render with "e.trim is not a function" during de-duplication. The dedup key now coerces non-string date and place values to text. Reported by [@tenephor](https://github.com/tenephor).

**Fix: Parent place dropdown no longer shows the raw type id for the highest-ranked type** ([#739](https://github.com/banisterious/obsidian-charted-roots/issues/739)): When a place's type is the highest in its hierarchy (so nothing can be its parent), the dropdown's "No valid parent types for …" message showed the internal sluggified id. It now shows the type's display name — a spot missed by #732. Reported by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Changed: Place type reordering now preserves intentional ties and gaps** ([#734](https://github.com/banisterious/obsidian-charted-roots/issues/734) follow-up): The up/down controls in the Place type manager now swap a type's hierarchy level with its neighbour's, rather than renumbering the whole category. Default same-rank pairs (State/Province, District/Township, and the like) stay tied, and any custom gaps are left intact. A tied neighbour disables the arrow, since tied types can't be ordered relative to each other — adjust one via Customize. The Customize/Edit action is now a gear/pencil icon with a tooltip to keep the row uncluttered. Raised by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Security: Bundled dompurify updated to 3.4.10** (a transitive dependency of jspdf, used for PDF export) to resolve advisories [GHSA-vxr8-fq34-vvx9](https://github.com/advisories/GHSA-vxr8-fq34-vvx9) and GHSA-gvmj-g25r-r7wr, both affecting dompurify `<= 3.4.8`.

**Stability-window impact:** v0.22.72's changes are bug fixes plus a reorder refinement and a dependency security bump (the org-member legacy-shape fix, the bare-number timeline fix, the parent-dropdown display name, and the tie-preserving place-type reorder), with no breaking changes, all within the v0.22.59 stability window, so the window holds at its v0.22.59 anchor (2026-06-02).

### v0.22.71 Round-Up: Place Type Management Overhaul and Display Names Everywhere (v0.22.71)

A reporter-driven release focused on place type management and consistent display names. The Place type manager (Control Center → Places) gains up/down reordering for both categories and the types within them, plus a per-category "+ Add type" button with smart hierarchy-level defaults — making non-Earth and deeply nested place hierarchies far less fiddly to build. Several surfaces that showed a raw internal id now show the proper display name, the Statistics date range splits universe-less fictional dates into their own "Uncategorized" bucket, and adding a member to an organization no longer silently fails when a person note carries a foreign `type` property. **1526 tests passing across 134 suites**.

**Added: Smarter hierarchy ranks for place types** ([#734](https://github.com/banisterious/obsidian-charted-roots/issues/734)): Building a multi-level place hierarchy (e.g. Region (space) → Sector → System for sci-fi worldbuilding) is much less fiddly. Each category in the Place type manager now has a "+ Add type" button that pre-selects that category and defaults the new type one hierarchy level deeper than the deepest existing type in it — so successive adds auto-increment instead of all landing at the same level. Each type row also has up/down arrows to reorder it within its category; moving renumbers the category's levels (anchored at its current minimum), which both realizes the chosen order and breaks ties between types that previously shared a level. Raised by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Added: Reorder place type categories with up/down controls** ([#733](https://github.com/banisterious/obsidian-charted-roots/issues/733)): The Place type manager now has move-up/move-down buttons on each category. Previously a custom category couldn't be sorted above the built-in categories — they occupy fixed sort positions a numeric order couldn't out-rank — so e.g. an "Astrographical" category was stuck below Geographic and Political divisions. Moving a category now renumbers the whole list, so any category (custom or built-in) can be placed anywhere, including at the very top. Raised by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Changed: Quick level presets get breathing room** in the Create/edit place type modal, instead of sitting flush against the dynamic hierarchy text above them. Minor spacing polish raised in discussion #728 by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Fix: Adding a member to an organization no longer silently fails when the person note has a foreign `type` property** ([#738](https://github.com/banisterious/obsidian-charted-roots/issues/738)): If a person note carried a `type` key (e.g. `type: character` from a user's own data model) alongside `cr_type: person`, adding them to an organization updated the person's memberships but left the organization's own member list empty — with no error. The organization's member scan used an ad-hoc check that excluded any note whose `type` wasn't exactly "person". The scan now uses the shared note-type detection, where `cr_type` is authoritative and `type` is only a fallback. Reported by [@prayidae](https://github.com/prayidae).

**Fix: Organization and source types now show their display name in the Entity Profile pane** ([#735](https://github.com/banisterious/obsidian-charted-roots/issues/735)): The profile pane's identity header showed the Organization type and Source type as their raw sluggified id (e.g. `noble_house` for House Organa) rather than the display name. Both now resolve to the proper name — honoring built-in names and customizations — while editing still edits the underlying id. Same class of issue as #665 (which fixed it for event types in the same pane); surfaced in discussion #730 by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Fix: Place types now show their display name instead of the internal id across the Control Center and place modals** ([#732](https://github.com/banisterious/obsidian-charted-roots/issues/732)): The Place Statistics card's type breakdown, the Places list's Type column, and the Create/Edit place modal's Parent place dropdown headers all rendered the raw sluggified id (e.g. `space_region`, `astro_system`) instead of the type's display name. Each now resolves to the proper name — honoring built-in names and customizations — so they read "Region (space)", "Astro system", and so on; an unknown id falls back to a humanized form of the slug. Same class of issue as #731, surfaced in discussion #728 by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Fix: Universe dynamic blocks now show the type's display name instead of its internal id** ([#731](https://github.com/banisterious/obsidian-charted-roots/issues/731)): The Type column in the `charted-roots-universe-places`, `-events`, and `-organizations` blocks rendered the raw sluggified id (e.g. `astro_sector`, `space_region`, `noble_house`, `plot_point`) rather than a readable label. Each type id now resolves to its proper display name — honoring built-in names and any customizations — so the column reads "Astro sector", "Noble house", and so on. An unknown id (such as a since-deleted custom type) falls back to a humanized form of the slug. Reported by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Fix: Universe-less fictional dates no longer skew the real-world Statistics range** ([#719](https://github.com/banisterious/obsidian-charted-roots/issues/719) follow-up): A person with a BBY/ABY (or other era) date but no universe link still resolves to a signed canonical year, which the per-universe Entity overview pooled into the real-world span — showing a bare `-33 — 9` where Gregorian years belong. Such dates now group under their own Uncategorized range instead of dragging the real-world (and Collections / folder-statistics) span into nonsense, signalling that those entities need a universe assigned. Raised by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Stability-window impact:** v0.22.71's changes are additive items (the place-type "+ Add type" defaults and category/type reordering) plus bug fixes (the foreign-`type` org-member fix, the display-name resolution across the Entity Profile, Control Center, place modals, and universe blocks, and the Uncategorized statistics bucket), with no breaking changes, all within the v0.22.59 stability window, so the window holds at its v0.22.59 anchor (2026-06-02).

### v0.22.70 Round-Up: Calendarium Integration Fix and Settings Relocation, Place-Linking by Note Type, and Era-Aware Statistics (v0.22.70)
...
A reporter-driven cohort focused on Dynamic Timeline Blocks, the Family Chart, and import. Timelines now handle fictional calendars properly: context-note events written in eras like BBY/ABY display, and a timeline that mixes eras (BBY and ABY) sorts and filters by true chronology instead of the bare year number. Event icons appear consistently across timeline rows and are shown by default. The Family Chart no longer labels unmarried co-parents as spouses, and its focus indicator stands out more clearly. Children added from a parent's Edit Person modal link back to the parent automatically, and the import wizard warns before creating a very large vault. **1361 tests passing across 117 suites**.
...
**Changed: Timelines show event icons by default** ([#691](https://github.com/banisterious/obsidian-charted-roots/issues/691)): The Event type display setting now defaults to "Icon with label" instead of "Text label", so timelines, canvas event nodes, and maps show their event-type icons out of the box. Existing vaults are migrated once on upgrade; if you prefer text only, set it back in Settings and your choice sticks. Reported by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Changed: Family Chart focus outline is more visible** ([#689](https://github.com/banisterious/obsidian-charted-roots/issues/689)): The focus (root person) outline added in v0.22.64 is now drawn slightly thicker (4px instead of 3px) so it stands apart from the connector lines, which share the accent colour — at the old width the default purple accent in dark mode could be easy to miss. Raised by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Fix: Timeline context notes work with fictional calendars** ([#693](https://github.com/banisterious/obsidian-charted-roots/issues/693)): A historical-context note added to a Dynamic Timeline Block displayed its events only when the dates were written as four-digit years. Lines dated in a fictional era — `BBY 32`, `32 BBY`, and the like — were silently skipped, so a fictional context note showed nothing while a real-world one worked. Context dates now recognise era abbreviations (in either order), short years, and ranges, and resolve through the same date system as the person's own events. Reported by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Fix: Mixed-era timelines sort and filter by true chronology** ([#695](https://github.com/banisterious/obsidian-charted-roots/issues/695)): The Dynamic Timeline Block ordered events, and applied the context lifespan margin, by the bare year number while ignoring the era. A timeline mixing BBY and ABY dates therefore ordered them meaninglessly (an `ABY 40` event could sink below a `BBY 10` one), and context-note events were kept or dropped by digit magnitude rather than their real distance from the person's life. Sorting and the margin window now use era-aware canonical years, so timelines read earliest-at-top consistently across eras (matching real-world dates); a pure-BBY timeline now also reads birth-at-top, like Gregorian timelines always have.

**Fix: Event icons appear consistently on timelines** ([#691](https://github.com/banisterious/obsidian-charted-roots/issues/691)): In "Text label" mode, a person's own events (birth, death, their event notes) showed an icon only when the same timeline also contained a family event, while historical-context and family rows always showed icons — so the same note could render icons on some rows but not others, and an identical event type appeared iconed for one person and bare for another. Event icons are now all-or-nothing per timeline, driven solely by the Event type display setting. Reported by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Fix: Family Chart no longer labels unmarried co-parents as spouses** ([#694](https://github.com/banisterious/obsidian-charted-roots/issues/694)): With kinship labels on, two people who share a child but have no spouse relationship were labelled "Spouse" — the label was inferred from the shape of the connecting line, and the chart draws a straight line between co-parents so their child can branch from it. The label is now checked against the actual relationship data, so only real spouses are labelled "Spouse" and an unmarried co-parent connector carries no label. Reported by [@doctorwodka](https://github.com/doctorwodka).

**Fix: Children added from a parent's Edit Person modal link back to the parent** ([#697](https://github.com/banisterious/obsidian-charted-roots/issues/697)): Creating (or linking) a child through a person's Edit Person modal listed the child on the parent's note but left the child's own note without a `father` / `mother` link, so the relationship was only half-connected until you ran the Data Quality bidirectional fix. The reverse parent link is now written onto each child when the parent is saved. Reported by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Fix: Family Chart highlight dimming no longer clips connector lines on Circle cards** ([#670](https://github.com/banisterious/obsidian-charted-roots/issues/670)): With Highlight Groups active and the Circle card style, the dim layer on non-matching cards was sized to a bounding box, which overhung the round card and faintly dimmed the connector lines passing near it. The dim layer now matches the card's circle, so the lines stay clean. Follows up the v0.22.63 highlight-dimming fix. Reported by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Stability-window impact:** v0.22.65's changes are bug fixes plus two additive items (the large-import warning and showing event icons by default), all within the v0.22.59 stability window, so the window holds at its v0.22.59 anchor (2026-06-02).

### v0.22.64 Round-Up: Family Chart Focus Indicator, Per-Move Migration Routes, and GEDCOM Place Consolidation (v0.22.64)

A reporter-driven release across the Family Chart, migration statistics, and GEDCOM import. The Family Chart's focus indicator is visible again, and highlight groups now work when the chart is in a pop-out window. Migration routes count each move a person made rather than only their net birthplace-to-final journey, so shared family moves and round trips are represented. And GEDCOM import folds variant spellings of the same place into one note instead of creating duplicates, with historical place names now stored as flat properties. **1326 tests passing across 113 suites**.

**Fix: Family Chart focus indicator is visible again** ([#689](https://github.com/banisterious/obsidian-charted-roots/issues/689)): The border drawn around the focus (root) person's card took its colour from the card's text, so the darker card text introduced in v0.22.63 made the focus outline blend into dark cards and effectively disappear. The outline now uses the theme's accent colour, independent of the text colour, so the focused person is clearly marked, in the chart and in exports. Reported by [@tenephor](https://github.com/tenephor).

**Fix: Family Chart highlight groups work in a pop-out window**: With the Family Chart moved to its own window, Highlight Groups had no effect — cards were neither dimmed nor highlighted — even though selecting a card still worked. The highlight pass was looking for cards in the main window rather than the pop-out, so it found none. It now applies within the chart's own window. Raised in the [#670](https://github.com/banisterious/obsidian-charted-roots/issues/670) discussion by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Changed: Migration routes count each move, not just the net journey** ([#684](https://github.com/banisterious/obsidian-charted-roots/issues/684)): "Top migration routes" (Statistics Dashboard and Places) previously modeled each person as a single origin-to-destination pair — their first known place to their last — which undercounted shared moves and hid round trips. A family that gathers at one place and moves on now counts every member on the shared leg even when they were born in different places, a round trip contributes both of its legs instead of vanishing, and intermediate stops appear as their own routes. The migration rate and moved totals are unchanged, and the parent/child place collapse still applies per leg so a place and its own sub-place don't read as a move. Follows up [#643](https://github.com/banisterious/obsidian-charted-roots/issues/643). Requested by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Changed: GEDCOM import consolidates variant spellings of the same place** ([#687](https://github.com/banisterious/obsidian-charted-roots/issues/687)): A single place that appeared under several `PLAC` forms across a GEDCOM (for example "Montréal, Québec, Canada", "Montreal, Quebec, Canada", and "Montreal, QC, Canada") used to create a separate place note for each, fragmenting everything attached to that location. Import now folds these into one note: Canadian province abbreviations expand like US states already did (QC to Quebec, Ont. to Ontario), and accent / case / spacing / punctuation variants and "same town under a differently-named higher tier" forms consolidate, keeping the richest spelling and preserving the other forms in `historical_names`. The consolidation is deliberately conservative — it never merges two places that only share a leaf name, so Quebec the province and Quebec the city stay separate. Reported by [@inerlogic](https://github.com/inerlogic).

**Changed: Historical place names are stored as flat properties** ([#687](https://github.com/banisterious/obsidian-charted-roots/issues/687)): `historical_names` was a nested YAML array of objects, which Obsidian does not fully support (the Data Quality pane flagged it, like the image-crop case in v0.22.63). It is now a flat list of names with an optional index-aligned `historical_name_periods` list. Existing notes still load (the older nested form is read for backward compatibility), and **Flatten nested properties** (Control Center) now offers a "Historical place names" option that converts them in bulk. The Merge Duplicate Places tool and the place template write the flat form too.

**Stability-window impact:** v0.22.64's changes are two fixes plus three enhancements (per-move migration routes, place-variant consolidation, and flat historical names), all within the v0.22.59 stability window, so the window holds at its v0.22.59 anchor (2026-06-02).

### v0.22.63 Round-Up: Suffix-Aware Imports, Place-Unlink Fix, Migration Refinements, and Flat Crop Data (v0.22.63)
...
**Fix: Statistics Dashboard issues card lists only the categories with something to fix** ([#676](https://github.com/banisterious/obsidian-charted-roots/issues/676)): The dashboard's summary "Issues" card always spelled out "Missing births + orphans + unsourced events" in its subtitle, even when one of those categories had a zero count — so with birth dates fully complete it still read "Missing births" despite none being missing. The subtitle now names only the categories that actually contribute to the count. Follows up the v0.22.61 fix. Reported by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Fix: Split canvas wizard's completion screen lays out correctly** ([#673](https://github.com/banisterious/obsidian-charted-roots/issues/673)): The wizard's final "Complete" step rendered its created-files list and configuration summary crammed together and overlapping the step indicator, because that step's content used the wrong container style. It now uses the same column layout as every other step in the wizard. Follows up the v0.22.61 filename fix. Raised by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Fix: Family Chart Circle avatars keep their colored ring after "fit to view"** ([#677](https://github.com/banisterious/obsidian-charted-roots/issues/677)): With the Circle card style, clicking "fit to view" kept the avatars round but dropped the gender-colored ring behind each one. The fit re-render dropped the ring along with the circular clip, and only the clip was being restored afterward; the ring is now re-applied too. Follows up the v0.22.61 pop-out fix. Reported by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Stability-window impact:** v0.22.62's changes are three fixes plus a forward-compatibility update to the Obsidian 1.13.0 settings and button APIs (no visible change on current releases), all within the v0.22.59 stability window, so the window holds at its v0.22.59 anchor (2026-06-02).

### v0.22.61 Round-Up: Event-Driven Migration Analysis, Historical Place Names, and Family Chart Pop-Out Fixes (v0.22.61)
...
A focused follow-up to v0.22.58 closing a relationship data-integrity bug and two Family Chart rendering issues. A parent note's children list could fall out of step with its companion ID list and silently mis-pair or drop relationships on the next edit; this is now prevented at the source and repairable for already-affected notes through a new Data Quality tool. The Family Chart "Circle" card style is rebuilt to show round avatars with their connection indicators and placeholders intact, and the batch-repair warning callouts are now legible in every theme. **1162 tests passing across 96 suites**.

**Fix: Misaligned children lists no longer mislabel or drop relationships** ([#666](https://github.com/banisterious/obsidian-charted-roots/issues/666)): A parent stores its children as two parallel lists — `children` (the display wikilinks) and `children_id` (the stable IDs) — that must line up one-for-one. If an ID went missing (for example, a child link left in path form during earlier editing), the lists fell out of step, and the next edit re-paired each child's name with the wrong child's note, producing links like `[[correct note|wrong name]]` and dropping a reference. The Edit Person load path now pairs each link with its own resolved identity (rather than by position) whenever the two lists disagree in length, and the rename cascade identifies the renamed person by ID instead of array position, so neither path can introduce the mismatch. A new **"Repair misaligned children"** tool in the Data Quality tab detects affected parent notes and rebuilds their children list from each child's own parent links (the authoritative reciprocal side), recovering dropped children, clearing the misleading aliases, and removing links that resolve to no note — with a preview of every change before anything is written. Reported by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Fix: Family Chart "Circle" card style shows connection indicators and the photo placeholder again** ([#669](https://github.com/banisterious/obsidian-charted-roots/issues/669)): The Circle card style was drawn with a different internal renderer than the other styles, which never produced the small connection-indicator bubbles (the markers showing a card has more relatives to expand) and rendered the no-photo placeholder incorrectly. Circle now renders a round avatar — ringed in the gender color, name centered beneath — on the same renderer the other card styles use, so the bubbles, the placeholder, and branch expand/collapse behave consistently across every theme. Reported by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Fix: Data Quality warning callouts are readable in every theme**: The "back up your vault" notice shown before batch repairs rendered warning-colored text on a warning-colored background, which could disappear entirely in some themes; it now uses the standard text color so the message and its icon stay legible.

**Stability-window impact:** #666 is a data-loss-class fix — a parent's children could be silently mis-paired or dropped — so it resets the stability window, and v0.22.59 becomes the new anchor (2026-06-02). This is the eighth reset of the 0.22.x run, each one a bug caught before 1.0 rather than after. The post-release Community Plugins automated review surfaced one advisory CSS warning (`clip-path` partial-support, from the Circle card rebuild), addressed in a follow-up.

### v0.22.58 Round-Up: Reporter Follow-Ups, a Sibling-Marriage Toggle, and High Contrast Lines (v0.22.58)

A reporter-driven follow-up to v0.22.57, re-fixing several issues whose first pass had missed a parallel render or save path — fictional-date calendar attribution, event participant and place aliases, the timeline export person filter, and the Create/Edit Event modal's Organizations field — alongside a Data Quality refresh fix, an event-profile header label fix, a Family Chart High Contrast line-visibility fix, and a new opt-in "Show sibling's marriages" timeline toggle. **1145 tests passing across 95 suites**.

**New: "Show sibling's marriages" timeline toggle** ([#661](https://github.com/banisterious/obsidian-charted-roots/issues/661)): A new opt-in toggle adds a sibling's marriage to the focal person's Dynamic Timeline block — so you can see the focal person's age at a sibling's wedding — joining the existing relative-event toggles for parents' ([#608](https://github.com/banisterious/obsidian-charted-roots/issues/608)) and children's ([#607](https://github.com/banisterious/obsidian-charted-roots/issues/607)) marriages. Marriages are read from the same person-encoded vital-event data, cover biological and adopted siblings (step-siblings excluded, matching sibling births), and carry the spouse's name, marriage location, and a customizable label. Off by default. Requested by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Fix: A universe's default calendar drives how its dates are read** ([#650](https://github.com/banisterious/obsidian-charted-roots/issues/650)): Fictional dates in a note assigned to a universe now resolve through that universe's chosen default calendar, and a custom calendar is preferred over a built-in that shares its era abbreviations (e.g. `BBY` / `ABY`). The v0.22.57 fix still credited the built-in when the abbreviations collided on some configurations; attribution now honors the universe note's default calendar, so the "Systems in use" breakdown shows the right calendar. Reported by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Fix: Person-filtered Canvas and Excalidraw timeline exports return the right events** ([#657](https://github.com/banisterious/obsidian-charted-roots/issues/657)): The v0.22.57 fix normalized the timeline export's "Filter by person" comparison for the on-screen preview and the Markdown export, but the Canvas and Excalidraw exporters — the default format — still compared the bracket-stripped dropdown value against event person fields that keep their brackets, so a person-filtered Canvas export failed with "No events to export after filtering." All three formats now share a single person-matcher. Reported by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Fix: The Create/Edit Event modal saves the Organizations field** ([#659](https://github.com/banisterious/obsidian-charted-roots/issues/659)): The "Organizations" field added in v0.22.57 was never wired into either save path, so entering organizations and saving reported success while writing nothing — only hand-editing the frontmatter worked. The field now persists on both create and edit and pre-fills when editing, and the organization profile's "Events" section shows each event's type by its proper name and icon instead of the raw slug. Reported by [@doctorwodka](https://github.com/doctorwodka) and [@DigitalDreamn](https://github.com/DigitalDreamn).

**Fix: Event tables and timelines show participant and place aliases** ([#658](https://github.com/banisterious/obsidian-charted-roots/issues/658)): The v0.22.57 fix corrected the event Entity Profile's Participants section, but the event tables elsewhere — the dockable Events view, the Control Center timeline tables, the person-filter dropdowns, and the place line on the person-profile timeline — still stripped only the wikilink brackets, so an aliased participant or place rendered as `Filename|Alias`. These now resolve through the shared display helper, showing the alias while still linking to the correct note. Reported by [@doctorwodka](https://github.com/doctorwodka).

**Fix: Event profile header shows the event type's display name** ([#665](https://github.com/banisterious/obsidian-charted-roots/issues/665)): The Entity Profile heading listed an event's type as its internal slug (e.g. `plot_point`) instead of the catalog name ("Plot point"). The header now resolves the type through the event-type catalog, built-in and custom alike, while keeping the raw ID as the editable value. Reported by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Fix: Data Quality pane reflects corrected data after an edit** ([#664](https://github.com/banisterious/obsidian-charted-roots/issues/664)): After filling in missing data (for example a birth date), the Data Quality view — and its Refresh button — could keep reporting the item as missing, because it re-scanned on the file-save event, which fires before Obsidian re-reads the note's frontmatter. It now refreshes when the note's metadata has actually been re-read, so corrected values clear from the list on their own shortly after editing. Reported by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Fix: Family Chart connector lines stay visible in the High Contrast theme** ([#668](https://github.com/banisterious/obsidian-charted-roots/issues/668)): The High Contrast theme sets its text color to black so card labels read against the bright card fills, but the connector lines between cards were stroked with that same color and vanished against the theme's black background. The line color is now derived to stay visible against the chart background — each theme keeps its tuned line color when it already contrasts the background, and a visible tone is substituted only when it would not. Reported by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Stability-window impact:** v0.22.58 is the twelfth patch in the v0.22.46-anchored stability window opened on 2026-05-17. A reporter-driven cohort of UX fixes and one opt-in feature with no data-loss-class changes; doesn't reset the window. Post-release Community Plugins automated review held at the v0.22.53 baseline — no new findings.

### v0.22.57 Round-Up: Reporter Cohort — Fictional-Date Parsing, Organization Linking, and Profile Sections (v0.22.57)

A reporter-driven cohort closing eleven issues from the @doctorwodka and @DigitalDreamn threads, spanning fictional-date handling, the Events tab statistics, the Entity Profile views, and organization linking. Three new capabilities land — organization↔event links, a "Referenced events" section on source profiles, and a "Dissolved" field on the Organization modal — alongside fixes for fictional-date counting and parsing (negative years, decade notation, custom-calendar attribution), the timeline export person filter, the cross-tab "Manage media" action, and several Control Center papercuts. **1106 tests passing across 88 suites**.

**New: Link organizations to events** ([#659](https://github.com/banisterious/obsidian-charted-roots/issues/659)): Event notes can now reference organizations through an `organizations` wikilink array, and those events appear in the "Events" section of each linked organization's Entity Profile. Previously the organization profile gathered events via a placeholder that only matched when a person happened to be named like the organization, and the FAQ's advice to use the `groups` property didn't work (`groups` is a string filter, not an entity link). The Create/Edit Event modal gains an "Organizations" field, and the event service writes the links as wikilinks and exposes a `getEventsForOrganization` reverse-lookup mirroring how events already link to persons and places. Requested by [@doctorwodka](https://github.com/doctorwodka).

**New: "Referenced events" section on the Source profile** ([#654](https://github.com/banisterious/obsidian-charted-roots/issues/654)): A source note's Entity Profile now lists the event notes that cite it (via their `sources` array), alongside the existing "Referenced facts" and "Media" sections, with each entry linking through to the event. The loader reuses the same basename match as the referenced-facts reverse-lookup. Requested by [@DigitalDreamn](https://github.com/DigitalDreamn).

**New: "Dissolved" field in the Create/Edit Organization modal** ([#649](https://github.com/banisterious/obsidian-charted-roots/issues/649)): The `dissolved` organization property was already supported by the templates, profile view, and frontmatter reference, but the only way to set it was hand-editing YAML. The modal now has a "Dissolved" input directly under "Founded", and both the create and update paths serialize it to the note's frontmatter. Requested by [@doctorwodka](https://github.com/doctorwodka).

**Fix: Negative years and decade notation parse and sort correctly** ([#655](https://github.com/banisterious/obsidian-charted-roots/issues/655), [#660](https://github.com/banisterious/obsidian-charted-roots/issues/660)): The fictional-date parser rejected signed/negative years (`EP -18`), and its ISO month/day suffix strip corrupted them — matching the `-18` as if it were a month suffix and eating the year — so negative-dated events fell back to a broken text sort and rendered out of order (`-01-12` before `-200` before `-500`). Negative years now parse to a negative canonical year and sort earliest-first (`-500` before `-200` before `-1`); the suffix strip only fires when a year digit precedes it (so `DE 1222-03` still reduces to `DE 1222` while `EP -30` is left intact); and decade notation (`EP 30s`, `EP -30s`) parses to the decade's start year flagged approximate. Because every fictional-date surface routes through the parser, this also corrects era display, age annotations, and the Events tab statistics for these shapes. Reported by [@doctorwodka](https://github.com/doctorwodka).

**Fix: Statistics and Timeline Export count fictional dates correctly** ([#648](https://github.com/banisterious/obsidian-charted-roots/issues/648)): Two counters on the Events tab undercounted fictional dates because they used local date helpers stricter than the parser the rest of the plugin uses. The Statistics card's "notes use fictional date systems" count skipped approximation markers (`DE ~310`), ISO-style month/day suffixes (`DE 1264-08`), and a person whose only fictional date was a death (no birth); the Timeline Export's "N dated" detection only matched a leading four-digit year, so an all-fictional vault read "0 dated." Both counters now route through the shared date service. Reported by [@doctorwodka](https://github.com/doctorwodka) and corroborated by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Fix: "Systems in use" attributes dates to the correct custom calendar** ([#650](https://github.com/banisterious/obsidian-charted-roots/issues/650)): When a custom calendar reused a built-in system's era abbreviations (e.g. a custom Star Wars calendar sharing `BBY` / `ABY` with the built-in Galactic Standard Calendar), the "Systems in use" breakdown credited the built-in and the custom system never appeared. Attribution now resolves through the shared parser using the note's linked universe, so the custom system wins on a collision. Reported by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Fix: Event profile shows the participant's alias** ([#658](https://github.com/banisterious/obsidian-charted-roots/issues/658)): On an event note's profile, a participant linked with an aliased wikilink (displayed name differing from the filename — common after correcting a misspelling) rendered the raw link text instead of the alias. The Participants section now displays the alias while still resolving to the correct note when clicked. Reported by [@doctorwodka](https://github.com/doctorwodka).

**Fix: Timeline export "Filter by person" matches the selected person** ([#657](https://github.com/banisterious/obsidian-charted-roots/issues/657)): Choosing a person in the timeline export's "Filter by person" dropdown collapsed the preview to "0 events" — the filter compared the bracket-stripped dropdown value against event person fields that keep their brackets. Both sides are now normalized, so filtering returns that person's events and the count and date range update accordingly.

**Fix: "Manage media" works across all Control Center entity tables** ([#656](https://github.com/banisterious/obsidian-charted-roots/issues/656)): The "Manage media…" action on the People, Places, Organizations, and Events tabs (and in the Media Gallery) called a plugin method that doesn't exist, throwing at runtime and silently doing nothing. The call sites now invoke the shared helper, so the manage-media modal opens from every entity surface.

**Fix: Statistics recompute once the metadata cache settles** ([#651](https://github.com/banisterious/obsidian-charted-roots/issues/651)): The Statistics card's counts are computed by walking Obsidian's metadata cache, which can be incompletely populated at first render (cold start, post-update, mid-reindex), so the card could briefly show a low number until a manual tab reopen. It now recomputes when Obsidian signals the cache has settled. Defensive fix for a hard-to-reproduce timing window. Reported by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Fix: "View full statistics" surfaces the dashboard in front of the modal** ([#645](https://github.com/banisterious/obsidian-charted-roots/issues/645)): On the Events, Sources, Organizations, and Places tabs, the "View full statistics →" link opened the dashboard in a background leaf behind the Control Center modal, so it looked like nothing happened. These tabs now close the Control Center first, matching the Maps, People, Universes, and Dashboard tabs that already did. Spotted by [@DigitalDreamn](https://github.com/DigitalDreamn).

**Stability-window impact:** v0.22.57 is the eleventh patch in the v0.22.46-anchored stability window opened on 2026-05-17. A reporter-driven cohort of UX fixes and additive features with no data-loss-class changes; doesn't reset the window. Post-release Community Plugins automated review held at the v0.22.53 baseline — no new findings.

### v0.22.56 Round-Up: Fictional-Date Event Count and jspdf Bundle Patch (v0.22.56)

Small reactive patch closing one reporter-driven Events tab counting bug ([#644](https://github.com/banisterious/obsidian-charted-roots/issues/644)) and stripping a dynamic-script-loading code path out of jspdf's bundled output that the v0.22.55 Community Plugins automated review flagged in error. The fictional-date count on the Events tab Statistics card now reflects event notes alongside person notes rather than person notes only, so a vault of mostly-event fictional dates no longer reads as "0 notes use fictional date systems." A new `patch-jspdf.js` postinstall script removes jspdf's unreachable `pdfobjectnewwindow` output mode (the only `document.createElement('script')` call in the bundled main.js) so the next Community scan clears the error. **1064 tests passing across 82 suites**.

**Fix: Events tab Statistics card counts event notes alongside person notes for the fictional-date total** ([#644](https://github.com/banisterious/obsidian-charted-roots/issues/644)): On the Control Center's **Events** tab, the Statistics card's "Fictional dates" subsection said "X notes use fictional date systems" but the X only counted *person* notes with fictional `born` or `died` dates — event notes' `date` field was never inspected, despite the label promising a count of "notes". [@doctorwodka](https://github.com/doctorwodka) caught it on two vaults: in one, the card said 9 when she actually had 19 events with fictional dates; in the other, the card said 0 even though she had 3 events with fictional dates (and 0 fictional-dated persons). The fix extends `calculateDateStatistics` to dispatch to either the person branch or a new event branch in the same pass — event notes' `date` field (resolved through the property-alias service so user-defined aliases like `event_date` are honored) is checked with the same `looksLikeFictionalDate` predicate that the person branches already use, and a match increments `withFictionalDates` and the per-system breakdown one-for-one alongside the person counts. The "Systems in use" line picks up event-detected systems automatically. The label stays accurate now that the count reflects both kinds of notes. Dev-vault verification: count went from 37 (persons only) to 50 (persons + 13 fictional-dated events).

**Security: Strip jspdf's `pdfobjectnewwindow` output mode from the bundled ES build**: The v0.22.55 dep bump cleared the 18 dependency advisories that the v0.22.54 Community Plugins automated review surfaced, but introduced a new finding the next scan picked up: dynamic `<script>` element creation in the bundled main.js. The source: jspdf 4.x's `output('pdfobjectnewwindow')` mode opens a new window and dynamically loads `pdfobject.min.js` from a CloudFlare CDN via `document.createElement('script')`. It's the only `createElement('script')` call in jspdf's source. The plugin only ever calls `save()` (Family Chart export, Tree wizard PDF output); the `pdfobjectnewwindow` and `pdfjsnewwindow` output modes are never reached. A new `patch-jspdf.js` postinstall script replaces the unreachable case body with a `throw`, preserving the switch shape so jspdf still parses cleanly while dropping the dynamic script element creation from the bundle. The patch follows the project's existing postinstall-patching pattern (alongside `patch-family-chart.js`, `patch-leaflet-distortable.js`, `patch-core-js-polyfill.js`, and `patch-pdfmake.js`), carries an idempotency marker so re-runs are no-ops, and skips gracefully if jspdf vendors an updated source. The bundled `main.js` script element creation count drops from four sites to three (the remaining three from pdfmake, canvg, and leaflet-distortableimage have been present across all post-v0.22.48 releases and have never been flagged by the scanner). Tests, ESLint, and Stylelint baselines unchanged.

**Stability-window impact:** v0.22.56 is the tenth patch in the v0.22.46-anchored stability window opened on 2026-05-17. Closes the v0.22.54-v0.22.55-v0.22.56 dependency-security arc (v0.22.54 surfaced 18 jspdf + dompurify advisories; v0.22.55 cleared them via a dep bump but introduced a new dynamic-script finding; v0.22.56 strips the unreachable jspdf code path that carried it). Post-release Community Plugins automated review returned to the prior v0.22.53 baseline — no new findings, no remaining dependency or behavior errors.

### v0.22.55 Round-Up: Dependency Security Bump (jspdf and dompurify) (v0.22.55)

Security-only patch addressing all 18 dependency advisories surfaced by the v0.22.54 Community Plugins automated review — the first time dependency findings have hit a Charted Roots release. `jspdf` upgrades from 3.0.4 to 4.2.1, which transitively brings `dompurify` from 3.3.0 to 3.4.7. No application-code changes; PDF export verified end-to-end on both surfaces (Family Chart export and Tree wizard PDF output) in dev-vault. **1064 tests passing across 82 suites**.

**Security: Bump `jspdf` to 4.2.1 and `dompurify` to 3.4.7**: The v0.22.54 Community scan flagged 10 advisories against `jspdf` direct and 8 against `dompurify` transitively via `jspdf`. The `jspdf` advisories cover Path Traversal in the Node `fs` build, HTML Injection in `output` methods, PDF Injection in the `AcroForm` module, DoS via malformed BMP and GIF dimensions, stored XMP metadata injection, race conditions in `addJS`, and PDF Object Injection via free-text color. The `dompurify` advisories cover XSS, prototype pollution, `FORBID_TAGS` bypasses via `ADD_TAGS` function predicates, and `SAFE_FOR_TEMPLATES` bypasses in `RETURN_DOM` mode. Upstream confirmed no breaking API changes through the 3.x and 4.x majors: jsPDF 3.0 dropped IE support, and jsPDF 4.0's only behavioral change is `fs` access restriction in the Node build, which doesn't apply to Obsidian's renderer context. The plugin's PDF surfaces — Tree Preview and Family Chart export — use only long-stable public APIs (`new jsPDF`, `addImage`, `addPage`, `save`, `setFontSize`, `setFont`, `setTextColor`, `text`, `addFileToVFS`, `addFont`), so the bump is drop-in. Both PDF flows were manually verified in dev-vault after the bump: Family Chart export's cover page renders with Roboto fonts as before, multi-page chart pages render identically, and the Tree wizard PDF output (right-click person → generate tree from context menu → PDF output) renders the expected tree image. The CHANGELOG entry catalogues each advisory's title and affected range.

**Stability-window impact:** v0.22.55 is the ninth patch in the v0.22.46-anchored stability window opened on 2026-05-17. Security-only dep bump; doesn't reset the window. Post-release Community Plugins automated review scheduled to re-run; restoration of the prior 96 / 100 baseline depends on whether any new advisories landed against other deps in the meantime.

### v0.22.54 Round-Up: Statistics and Relationships Polish, Timeline Era-Aware Sort, and Settings Reorg (v0.22.54)

Reactive patch closing four reporter-driven UX papercuts from the v0.22.53 fortnight. The Statistics Dashboard's "Events by type", "Sources by type", and "Places by category" sections now route stats keys through their type catalogs so rows render with their configured display names rather than raw lowercase IDs (`plot_point`, `legal`, `uncategorized`, ...) ([#641](https://github.com/banisterious/obsidian-charted-roots/issues/641)). The Relationships tab Statistics card's "By type" and "By category" rows now lay out with the label and count visually separated rather than collapsed as `Child131` / `Spouse22` / `Family169` ([#640](https://github.com/banisterious/obsidian-charted-roots/issues/640)). On descending-era timelines (BBY / BC-style), same-year events with `before` / `after` constraints now respect the era's old-at-bottom direction, mirroring the era-inversion the rawDate tiebreak gained in v0.22.49 ([#638](https://github.com/banisterious/obsidian-charted-roots/issues/638)). And a Control Center settings reorg pulls **Event display** out of Canvas & Trees and **Default timeline context** / **Context lifespan margin** out of Advanced into a renamed **Events & timelines** section, so all event- and timeline-related controls live in one place ([#637](https://github.com/banisterious/obsidian-charted-roots/issues/637)). **1064 tests passing across 82 suites**.

**Fix: Statistics Dashboard's "Events by type", "Sources by type", and "Places by category" show display names** ([#641](https://github.com/banisterious/obsidian-charted-roots/issues/641)): The three by-type and by-category sections in the Statistics Dashboard passed each stats key straight through as the row label, so rows that came from slugified event-type IDs (`plot_point`, `backstory`, `immigration`), built-in source-type IDs, or canonical place-category IDs rendered as the raw identifier instead of the configured display name. Each section now routes the key through its catalog (`getEventType` with the user's custom event types and customizations; `getSourceType` with custom source types; `PLACE_CATEGORY_LABELS` for place categories). When an ID has no matching definition — orphan rows from deleted custom types, IDs that are categories rather than types like `legal`, or the `uncategorized` sentinel assigned to notes with no type frontmatter — the row falls back to the capitalized key (`other` → `Other`, `legal` → `Legal`, `uncategorized` → `Uncategorized`) rather than the raw lowercase ID, and the row still appears rather than vanishing. The catalog lookup is the same one used by the timeline renderer, the Create/Edit Event modal, the map controller, and the source picker, so any user-customized display names (renames in the catalog editor, the `Plot point` and `Backstory` examples from @DigitalDreamn's worldbuilding vault) flow through to the dashboard without code changes. Raised by [@DigitalDreamn](https://github.com/DigitalDreamn) in [Discussion #639](https://github.com/banisterious/obsidian-charted-roots/discussions/639) (initial scope was "Events by type"; she expanded the scope on the issue to also cover "Sources by type" and "Places by category" after the dev-vault verification surfaced the same shape on her other categories).

**Fix: Relationships tab Statistics card lays out rows with the label and count separated** ([#640](https://github.com/banisterious/obsidian-charted-roots/issues/640)): On the Control Center's **Relationships** tab, the Statistics card's "By type" and "By category" rows at the bottom of the page rendered with the label and count run directly together (`Child131`, `Spouse22`, `Family169`), instead of with the label and count visually separated. The cause: five of the seven CSS classes the rows referenced (`.crc-stats-list`, `.crc-stats-item`, `.crc-stats-label`, `.crc-stats-value`, `.crc-stats-swatch`) had never been defined — only `.crc-stats-section` and `.crc-stats-section-title` existed in the stylesheet. The fix adds the missing rules so each row lays out as a flex row with the color swatch on the left, the relationship type or category name in the middle, and the count right-aligned and semibold, matching the Statistics Dashboard's "Events by type" treatment. Raised by [@DigitalDreamn](https://github.com/DigitalDreamn) in [Discussion #639](https://github.com/banisterious/obsidian-charted-roots/discussions/639).

**Fix: Dynamic Timeline Block `sort_order` tiebreak respects descending-era timelines** ([#638](https://github.com/banisterious/obsidian-charted-roots/issues/638)): The v0.22.51 [#625](https://github.com/banisterious/obsidian-charted-roots/issues/625) fix added a same-year `sort_order` tiebreak so per-person Dynamic Timeline Blocks could respect `before` / `after` relationships on Events, but it skipped the era-inversion that v0.22.49's [#609](https://github.com/banisterious/obsidian-charted-roots/issues/609) had taught the rawDate tiebreak to. In a BBY / BC-style descending era the year sort correctly puts old-at-bottom, but the `sort_order` pair flipped opposite — so two same-year events with `before` / `after` constraints read in the wrong direction locally. Both tiebreaks now route through a shared `isBackwardEraPair` helper that returns true if either entry in the pair parses as a backward-direction fictional date, and both invert when the helper fires. Keeping the era check in one place stops the two tiebreaks from drifting apart if either changes later. 4 new test cases in `tests/timeline-sort-year-tiebreak.test.ts` fence the BBY chronological inversion, the BBY reverse double-flip back, a Gregorian control case (no inversion), and the no-dateService fallback path. Reported by [@DigitalDreamn](https://github.com/DigitalDreamn) — same reporter as the upstream [#609](https://github.com/banisterious/obsidian-charted-roots/issues/609) and [#625](https://github.com/banisterious/obsidian-charted-roots/issues/625) fixes she also surfaced.

**Change: Control Center settings reorg — Event display and timeline context live alongside the Timeline section** ([#637](https://github.com/banisterious/obsidian-charted-roots/issues/637)): The Event display subsection (with the **Event type display** setting that controls how event types appear on timelines, canvas event nodes, and maps) moves from Canvas & Trees to the top of the Timeline section. The **Default timeline context** and **Context lifespan margin** settings move from Advanced into a new **Context events** subsection within the same section, parallel to the existing **Family events on timelines** subsection. The section itself is renamed **Events & timelines** to reflect the broader scope. Sync Calendarium events stays in Advanced > Integrations as an integration toggle. No setting names or values change; the internal section identifier stays `timeline` so the open-state restore continues to recognize a previously-open section across upgrades. Raised by [@doctorwodka](https://github.com/doctorwodka) after spending 10 minutes looking for Event Display under Canvas & Trees rather than the Timeline section it conceptually belongs to.

**Stability-window impact:** v0.22.54 is the eighth patch in the v0.22.46-anchored stability window opened on 2026-05-17. Four reactive UX-polish fixes; doesn't reset the window. Post-release Community Plugins automated review surfaced 10 advisories against `jspdf` and 8 against `dompurify` (transitive via jspdf) — the first time dependency findings have hit a Charted Roots release. The new findings break the prior 96 / 100 streak (held across v0.22.48 through v0.22.53, seven consecutive releases). Addressed in v0.22.55 (dep bump).

### v0.22.53 Round-Up: Family Chart Overlay Self-Heal, Adoption Icon Continuity, and First External Contribution (v0.22.53)

Reactive patch closing a cluster of community-reported papercuts — and the project's **first accepted external code contribution**. The Family Chart's custom-relationship overlay now self-heals if it draws before the chart's cards finish animating into place, so a top endpoint that occasionally detached from its source card on refresh now snaps back when the cards truly settle ([#615](https://github.com/banisterious/obsidian-charted-roots/issues/615)). Adoption events render with the `heart-handshake` icon across every Dynamic Timeline surface, not just the focal person's own row that v0.22.52 fixed — the adoptive parent's `Adopted {name}` row and the adopted-sibling / adopted-grandchild `Adoption of {name}` rows now share the same icon and warm orange color ([#632](https://github.com/banisterious/obsidian-charted-roots/issues/632)). The duplicate-detection modal's "Not a duplicate" dismissals now persist across sessions instead of reappearing on every scan, matching what the wiki has long promised ([#633](https://github.com/banisterious/obsidian-charted-roots/issues/633)). The Calendar view filters events by year as well as month, so a person born in a later year no longer appears in an earlier year's same month ([#634](https://github.com/banisterious/obsidian-charted-roots/issues/634), contributed by [@MohammadYusif](https://github.com/MohammadYusif) in [#636](https://github.com/banisterious/obsidian-charted-roots/pull/636)). And the Merge Duplicate Places modal's Help link now points at the current wiki rather than returning a 404. **1060 tests passing across 82 suites**.

**Fix: Family Chart custom-relationship overlay self-heals if it draws before cards settle** ([#615](https://github.com/banisterious/obsidian-charted-roots/issues/615)): On some setups the overlay line's top endpoint could detach from its source card after a refresh, floating partway down toward the next-generation card. The overlay waits for card positions to stabilize before drawing (via a poll that requires three consecutive RAF frames of identical positions after at least one observed change), but if the chart's entrance animation briefly stalls mid-flight — long enough for three sampled frames to read as equal while the card is still mid-tween — the wait fires against an intermediate position, anchoring the endpoint where the card was rather than where it ends up. The fix adds a backstop: shortly after drawing, it captures the card positions and, after a 750ms beat, checks whether any card moved since; if so, the first render was premature, so it re-runs the stability poll and redraws once the cards have truly settled. Re-runs the poll rather than drawing on a fixed timer, which keeps it safe on large trees where a fixed timer could itself capture a mid-animation frame (the [#386](https://github.com/banisterious/obsidian-charted-roots/issues/386) hazard the poll was introduced to avoid). No effect in the common case where positions are already final. Validated via a forced mid-animation draw on a scratch branch (drift reproduced on demand, backstop snap-back observed). Reported by [@doctorwodka](https://github.com/doctorwodka); the fix is framed as "should fix — please confirm" since her exact stall timing isn't locally reproducible.

**Change: Adoption events on family timelines now use the adoption icon** ([#632](https://github.com/banisterious/obsidian-charted-roots/issues/632)): v0.22.52 ([#627](https://github.com/banisterious/obsidian-charted-roots/issues/627)) gave the focal person's own `Adopted` row the `heart-handshake` icon in warm orange, but the adoption events on the *family* timeline surfaces — the adoptive parent's `Adopted {name}` row and the adopted-sibling / adopted-grandchild `Adoption of {name}` rows (the latter two made always-on in v0.22.50 via [#623](https://github.com/banisterious/obsidian-charted-roots/issues/623)) — still rendered with the generic `users` family-event icon. The fix routes adoption family events through the built-in `adoption` event type so they pick up the same icon and color as the focal-own row, keeping the visual cue consistent across every Dynamic Timeline surface. The adopted child's *birth* row (shown when **Show adopted children's births** is enabled) keeps the generic icon, since it is a birth rather than an adoption. The icon-resolution logic is extracted to a `resolveFamilyEventIcon` helper that respects user customization of the adoption type (a custom override replaces the built-in icon and color as expected). 4 new test cases in `tests/timeline-family-adoption-icon.test.ts`. Raised by [@DigitalDreamn](https://github.com/DigitalDreamn) in her [#627](https://github.com/banisterious/obsidian-charted-roots/issues/627) close-out comment, noting the icon should extend to the sibling and grandparent surfaces from #623 for visual continuity.

**Fix: "Not a duplicate" dismissals now persist across sessions** ([#633](https://github.com/banisterious/obsidian-charted-roots/issues/633)): In the duplicate-detection modal, marking a pair "Not a duplicate" only removed it from the current scan — the action wasn't saved anywhere, so the same pair reappeared on the next scan, contrary to the wiki's promise that dismissals are remembered. The persistence layer was never built. Dismissed pairs are now stored in settings as order-independent `crIdA::crIdB` keys (sorted, so a pair stays dismissed regardless of which side the detector lists first) and filtered out of future scans. The modal's options gain a **Clear dismissed** button — shown only when dismissals exist — to undo a stale dismissal and let the pair resurface on the next scan. The order-independence is the crux: without it, a dismissal recorded when the detector listed the pair as (A, B) wouldn't match the same pair if the next scan listed it as (B, A). 4 new test cases in `tests/duplicate-dismissed-pair-key.test.ts` fence the order-independence and the cross-ordering set-match behavior. Reported by [@Darcylynn](https://github.com/Darcylynn) in [Discussion #631](https://github.com/banisterious/obsidian-charted-roots/discussions/631).

**Fix: Calendar view filters events by year as well as month** ([#634](https://github.com/banisterious/obsidian-charted-roots/issues/634), the project's first external contribution): Navigating the Calendar view to a specific month and year (for example February 1850) showed events from every year that shared that month — so a person born in February 2010 appeared on Feb 15, 1850, with their actual year and "years ago" count exposing the mismatch. `getEventsForMonth` accepted the displayed year as a parameter but only compared each event's month, never its year. The fix adds the year to the loop guard so only events matching both the displayed month and year render; signed years (negative for descending custom eras like BBY / BCE) compare cleanly with direct equality. 4 new test cases in `tests/calendar-data-service-year-filter.test.ts` fence cross-year exclusion, match inclusion, signed-year handling, and the day-unknown imprecise-event path. Reported by [@Darcylynn](https://github.com/Darcylynn) in [Discussion #631](https://github.com/banisterious/obsidian-charted-roots/discussions/631); fixed and tested by [@MohammadYusif](https://github.com/MohammadYusif) in [#636](https://github.com/banisterious/obsidian-charted-roots/pull/636) — the project's first accepted external pull request.

**Fix: Merge Duplicate Places Help link points at the current wiki**: The Help link in the modal header pointed at a placeholder wiki URL and returned a 404 when clicked. The link now uses the current `banisterious/obsidian-charted-roots` wiki path, matching every other in-plugin help link. Reported by [@Darcylynn](https://github.com/Darcylynn) in [Discussion #631](https://github.com/banisterious/obsidian-charted-roots/discussions/631).

**Stability-window impact:** v0.22.53 is the seventh patch in the v0.22.46-anchored stability window opened on 2026-05-17. Five reactive papercut fixes plus one external contribution; doesn't reset the window. Post-release Community Plugins automated review held at 96 / 100 — seventh consecutive release at this score (v0.22.48 through v0.22.53), no new findings.
