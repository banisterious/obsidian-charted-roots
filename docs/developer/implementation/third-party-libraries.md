...
- **`patch-pdfmake.js` is a sibling postinstall patch** (chained after `patch-core-js-polyfill.js`) that strips two `new Function("return this")()` sites from `node_modules/pdfmake/build/pdfmake.js`: the bundled core-js globalThis polyfill body and the webpack runtime's `__webpack_require__.g` initializer. Both fallback branches are unreachable in Obsidian's Electron runtime because the `typeof globalThis === "object"` early-return always fires. Closes the Community automated review's Dynamic Code Execution Recommendation that was flagged on v0.22.44; the patch follows the same exact-string locate + idempotency marker + fail-loud-on-vendor-update shape as `patch-core-js-polyfill.js`. Added in v0.22.45. See `patch-pdfmake.js` for the full pattern.
...
- **`patch-leaflet-distortable.js` is a postinstall patch** (chained from `patch-family-chart.js` in `package.json:scripts.postinstall`) that applies four stubs to `leaflet-distortableimage`'s bundled dist file:
  ...
  3. **Mapknitter export status-poll** (`_defaultHandleStatusRes`) is stubbed because the original implementation wraps `setInterval` around `fetch(req)` in the same closure to poll `export.mapknitter.org` for an export job's status URL. The plugin renders image overlays in-vault only and never invokes the export start flow, so the handler is unreachable. The literal `setInterval` + `fetch` correlation pattern was flagged at warning severity by the Community automated review's Behavior rule. Stubbed to a no-op. Added in v0.22.42.
  4. **Webpack-dev-server live-reload poll** is stubbed because the bundled `reloadApp` helper sets a `setInterval` that watches `window.location.protocol` and triggers a live reload — dead in production for the same reasons as stub 1 (the dev server is never running). Stubbed to a no-op `intervalId`. Added in v0.22.42.
