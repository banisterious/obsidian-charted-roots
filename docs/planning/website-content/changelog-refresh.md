...
**Pending port (next session):** v0.22.58 + v0.22.59 + v0.22.60 + v0.22.61 Round-Up spotlights + cluster-intro metric bumps (to sixty-two releases / 1240 tests, the #666 data-loss reset to a new v0.22.59 anchor, and the v0.22.59 clip-path advisory cleared in v0.22.60) await the chartedroots.com port session. chartedroots.com `/changelog/` is current through v0.22.57. **Note:** v0.22.61's post-release Community scan surfaced newly-flagged Source-code advisories (an ESLint directive error and Obsidian 1.13 API deprecations) slated for a v0.22.62 follow-up — fold the scanner-streak prose update in once that lands.
