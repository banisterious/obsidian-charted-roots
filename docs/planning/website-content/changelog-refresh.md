...
Thirteen releases across this cluster. Four same-day hotfixes (0.22.1 through 0.22.4) closed critical data-loss bugs surfaced by community testing. 0.22.5 through 0.22.12 brought the fictional-calendar work into shape, landed Phase 1 of the universe → calendar link, closed eight distinct surfaces where era-naive year extraction silently broke fictional dates, fully closed the lat/lng-only-on-pixel-CRS cluster across three journey-mode sites, hardened person-delete cleanup against broken wikilinks and single-relationship scalar serializations, mirrored marriage details bidirectionally between spouses (closing one of the long-standing create/edit-asymmetry gaps), and cascaded Universe-note renames across referencing entity notes end-to-end. Regression tests grew from 189 to 436 across the thirteen releases. Stability window has held since 0.22.4 (eight patches without a reset).
