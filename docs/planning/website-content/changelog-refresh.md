...
**Pending port (next session):** v0.22.58 + v0.22.59 + v0.22.60 Round-Up spotlights + cluster-intro metric bumps (to sixty-one releases / 1176 tests, the #666 data-loss reset to a new v0.22.59 anchor, and the v0.22.59 clip-path advisory now cleared in v0.22.60) await the chartedroots.com port session. chartedroots.com `/changelog/` is current through v0.22.57.
