...
**Pending port (next session):** the **v0.22.74 Round-Up** spotlight (drafted below) plus the cluster-intro updates that accompany it — release count seventy-four → seventy-five, test count 1569 / v0.22.73 → 1587 / v0.22.74, the new v0.22.74 narrative sentence, and the scanner-streak extension through v0.22.74's clean post-release scan. Standing **verify-before-publish** reminders: the stability-window anchor (still v0.22.59, no resets through v0.22.74) and the scanner-streak prose against the #411 tracking.
