...
**Pending port (next session):** v0.22.58 + v0.22.59 Round-Up spotlights + cluster-intro metric bumps (to sixty releases / 1162 tests, the #666 data-loss reset to a new v0.22.59 anchor, and the v0.22.59 clip-path scanner advisory) await the chartedroots.com port session. chartedroots.com `/changelog/` is current through v0.22.57.
