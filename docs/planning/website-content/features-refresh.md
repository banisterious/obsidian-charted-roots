...
**Status:** 🚀 Ported to chartedroots.com 2026-04-24, refreshed 2026-04-25 with v0.22.6 + v0.22.7 sentence-level updates (map popups + ages, journey-mode placeholder, pixel-coord journey paths, profile-view children labels, timeline spouse-deaths default + stepchild filter, person-delete cleanup, custom-relationships overlay z-order, universe-calendar wiring). Rebuilt `/features/` page under the track-based Option C structure. **Reviewed 2026-05-10 for v0.22.29** — no feature-page bullets needed; the seven fixes (#551, #552, #553, #554, #555, #556, #558) all close coverage gaps in features already documented (property aliases, bidirectional sync, step-relationship sync) rather than adding new surface. Same pattern as v0.22.25–v0.22.28. **Reviewed 2026-05-11 for v0.22.30** — no feature-page bullets needed; #559 (cr_id collision filter) and #560 (negative-year decade bucketing) are silent fixes in features already implied (wikilink resolution and Statistics decade aggregation), and the Family Chart XSS hardening is an internal render-path rebuild with no user-facing surface change. ESLint baseline cleanup is housekeeping. **Reviewed 2026-05-12 for v0.22.31** — no feature-page bullets needed; the fictional-date cluster (#562/#563/#564/#565/#566) closes coverage gaps in the fictional-date feature already documented (universe-calendar wiring), the BidirectionalLinker correctness fix closes a setup-honoring gap in the bidirectional-sync feature already documented, the pop-out window timer migration is internal correctness with no user-facing surface change, and the "Open command menu" -> "Open quick actions" rename is a label change to an existing feature (the categorized command launcher / Quick Actions modal). The Quick Actions modal isn't currently called out on the features page; it's a Control Center surface, and the rename only changes the keyboard-command identifier and the dashboard tile label.
